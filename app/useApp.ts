...
export type View = 'home' | 'dashboard' | 'calendar' | 'notifications';
...
export const useApp = () => {
    ...
    const handleCreateNotification = (message: string, type: Notification['type']) => {
    ...
    const handleSetCurrentRole = (role: UserRole) => {
        ...
        // When switching roles, always go back to dashboard from other views
        if (currentView === 'calendar' || currentView === 'notifications') {
            setCurrentView('dashboard');
        }
        setCurrentRole(role);
    };
